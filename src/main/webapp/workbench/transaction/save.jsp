	...
	<script type="text/javascript">
		$(function(){
			...
			var json = {
				...
				%>
						"<%=stage%>" : <%=possibility%>,  //此处不需要处理最后一个逗号 已经声明是json对象,浏览器会处理
				<%
					}
				%>
			...
			$("#searchActivity").keydown(function(event){
				if(event.keyCode == 13){
					$.ajax({
						url : "workbench/tran/getActivityListByName1.do",
						data : {
						...
						success : function(data){
							...
							$.each(data,function(i,n){
								html += '<tr>';
								html += '<td><input type="radio" name="choice" value="'+n.id+'"/></td>';
			...
			$("#confirmBtn").click(function(){
				var activityId = $("input[name=choice]:checked").val();
				$("#activityName").val($("#name" + activityId).html());
			...
			$("#searchContacts").keydown(function(event){
				if(event.keyCode == 13){
					$.ajax({
						...
						data : {
							"fullname" : $.trim($("#searchContacts").val())
						},
						type : "get",
						dataType : "json",
						success : function(data){
			...
			$("#confirmBtn1").click(function(){
				var contactsId = $("input[name=choice]:checked").val();
				$("#contactsName").val($("#f" + contactsId).html());
...
<%--							<tr>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
					<button type="button" class="btn btn-primary" id="confirmBtn1">确认</button>
				</div>
			</div>
		</div>
	</div>
	...
	<form action="workbench/tran/save.do" method="post" class="form-horizontal" id="tranForm" role="form" style="position: relative; top: -30px;">
		...
		<div class="form-group">
			...
			<label for="create-activitySrc" class="col-sm-2 control-label">市场活动源&nbsp;&nbsp;<a href="javascript:void(0);" id="showActivity"><span class="glyphicon glyphicon-search"></span></a></label>
			<div class="col-sm-10" style="width: 300px;">
				<input type="text" class="form-control" id="activityName" placeholder="请点击左方搜索" readonly>
				<input type="hidden" id="activityId" name="activityId"/>
			</div>
		</div>
		
		<div class="form-group">
			<label for="create-contactsName" class="col-sm-2 control-label">联系人名称&nbsp;&nbsp;<a href="javascript:void(0);" id="showContacts"><span class="glyphicon glyphicon-search"></span></a></label>
			<div class="col-sm-10" style="width: 300px;">
				<input type="text" class="form-control" id="contactsName" placeholder="请点击左方搜索" readonly>
